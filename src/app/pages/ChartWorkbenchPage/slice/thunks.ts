...
import {
  ...
  fetchDataViewsActionFromBe,
  fetchViewDetailActionFromBe,
  updateChartActionForBe,
...
export const fetchDataSetAction = createAsyncThunk(
  ...
  async (arg: ChartDataRequest, thunkAPI) => {
    ...
    let response;
    if (BE_MODE) {
      response = await fetchDataSetActionFromBe(arg);
    } else {
...
export const fetchDataViewsAction = createAsyncThunk(
  ...
  async (arg: { orgId }) => {
    let response;
    if (BE_MODE) {
      response = await fetchDataViewsActionFromBe(arg);
    } else {
      response = await request2<any[]>({
        method: 'GET',
        url: `views`,
        params: arg,
      });
...
export const fetchViewDetailAction = createAsyncThunk(
  ...
  async (arg: { viewId }) => {
    ...
    let response;
    if (BE_MODE) {
      response = await fetchViewDetailActionFromBe(arg);
    } else {
      response = await request2<View>({
        method: 'GET',
        url: `views/${arg}`,
      });
    }
...
export const refreshDatasetAction = createAsyncThunk(
  ...
  async (
    ...
    const requestParams = builder
      ...
      // qingyang add
      .addChartId(workbenchState.backendChartId)
      .build();
...
>('workbench/fetchChartAction', async arg => {
  if (arg?.chartId) {
    let response;
    if (BE_MODE) {
      response = await fetchChartActionFromBe(arg.chartId);
    } else {
...
export const updateChartAction = createAsyncThunk(
  ...
  async (
    ...
    let response;
    if (BE_MODE) {
      response = await updateChartActionForBe(arg.chartId, requestBody);
    } else {
      response = await request2<{
        data: boolean;
      }>({
