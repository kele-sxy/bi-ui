    ...
    const [{ isOver, canDrop }, drop] = useDrop(
      () => ({
        ...
        canDrop: (item: ChartDataSectionField, monitor) => {
          ...
          // qingyang add
          // console.log('ondrop', currentConfig?.key, item);
          if (BE_MODE) {
            // 维度
            if (currentConfig?.key === 'dimension') {
              // if (!item?.local || item?.joinField) {
              if (item?.joinField) {
                return false;
              }
              const isNotSameParty = items?.every(i => {
                const partyTag = `${i?.colName?.split('.')?.[0]}.`;
                return currentRowNames?.some(x => !x?.startsWith(partyTag));
              });
              if (isNotSameParty) {
                return false;
              }
            ...
            // 筛选
            if (currentConfig?.key === 'filter') {
              if (item?.joinField) {
                return false;
              }
            }
            // 字段
            if (currentConfig?.key === 'mixed') {
              if (!item?.local) {
                return false;
              }
            }
