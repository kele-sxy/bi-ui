...
import {
  fetchBoardListFromBe,
  fetchChartActionFromBe,
  fetchChartListFromBe,
  fetchDataSetActionFromBe,
} from 'app/adpter/reqAdapter';
...
export const getFolders = createAsyncThunk<Folder[], string>(
  ...
  async orgId => {
    let response;
    if (BE_MODE) {
      const response1 = await fetchChartListFromBe();
      const response2 = await fetchBoardListFromBe();
      response = {
        data: [...response1.data, ...response2.data],
      };
    } else {
      response = await request2<Folder[]>(`/viz/folders?orgId=${orgId}`);
    }
    console.log('qingyang getFolders', response?.data);
...
export const fetchVizChartAction = createAsyncThunk(
  ...
  async (arg: {
    ...
    let response;
    if (BE_MODE) {
      response = await fetchChartActionFromBe(arg.backendChartId);
    } else {
      response = await request2<Omit<ChartDTO, 'config'> & { config: string }>({
        method: 'GET',
        url: `viz/datacharts/${arg.backendChartId}`,
      });
    }
    console.log('qingyang fetchVizChartAction', response?.data);
    return {
...
export const fetchDataSetByPreviewChartAction = createAsyncThunk(
  ...
  async (
    ...
    const data = builder
      ...
      // qingyang add
      .addChartId(currentChartPreview?.backendChartId)
      .build();

    let response;
    if (BE_MODE) {
      response = await fetchDataSetActionFromBe(data);
    } else {
    ...
    console.log('qingyang', response?.data);
    return {
      backendChartId: currentChartPreview?.backendChartId,
