...
import {
  batchFetchChartInfoFromBe,
  fetchBoardDetailFromBe,
  fetchDataSetActionFromBe,
  updateDashboardForBe,
...
>(
  ...
  async ({ boardId, callback }, { getState, dispatch }) => {
    ...
    if (BE_MODE) {
      await updateDashboardForBe(dashBoard.id, updateData);
    } else {
      await request2<any>({
...
>(
  ...
  async ({ boardId, chartIds, boardType }, { getState, dispatch }) => {
    ...
    if (BE_MODE) {
      response = await batchFetchChartInfoFromBe(chartIds.join());
    } else {
    ...
    const { datacharts, views, viewVariables } = response.data;
