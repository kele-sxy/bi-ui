...
import {
  ...
  fetchDistinctDataFromBe,
} from 'app/adpter/reqAdapter';
import { BE_MODE, DownloadFileType } from 'app/constants';
import {
  DownloadTask,
  DownloadTaskState,
...
export const getDistinctFields = async (
  ...
  if (executeToken) {
    ...
    let response;
    if (BE_MODE) {
      response = await fetchDistinctDataFromBe(requestParams);
    } else {
      response = await request2<ChartDataSetDTO>({
        method: 'POST',
...
export async function fetchDataChart(id: string) {
  let response;
  if (BE_MODE) {
    response = await fetchChartActionFromBe(id);
  } else {
    response = await request2<ChartDTO>(`/viz/datacharts/${id}`);
  }
